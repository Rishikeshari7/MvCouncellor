...
const OurPsychologist = () => {
  return (
    ...
    <p className=" text-xl sm:3xl md:text-5xl font-bold  " >Our Psychologist</p>
    <p className=" text-lg sm:text-2xl font-bold text-white text-center " >1300+ Best Psychologist from India for Online Consultation</p>
    <div className=" w-11/12 sm:w-10/12 flex flex-col justify-center items-center mx-auto gap-5">
      <div className=" grid grid-cols-2 sm:flex w-full flex-wrap gap-4 items-stretch justify-between">
        {professionals.map((data) => (
          ...
          >
            <div
              className={`flex justify-center items-center p-5 sm:p-10 bg-custom-gradient rounded-full w-14 h-14 sm:w-24 sm:h-24 md:w-36 md:h-36 `}
            >
            ...
            <p className="font-normal text-center text-sm sm:text-xl  mx-1  mt-2">
              {data.name}
            </p>
          </div>
        ))}
